...
import {
  LabWineFormData,
  labWineSchema,
} from "../../types/galeryOne/labWineSchema";
...
export function LabWineScreen() {
  ...
  const {
    ...
    defaultValues: {
      data: new Date(),
      analista: nomeOperador || String(usuarioId),
      marca_tanque: "",
      lote: "",
      alcool: "",
      densidade: "",
  ...
  const onSubmit = async (data: LabWineFormData) => {
    ...
    try {
      const payload = {
        ...
        dtReferencia: data.data,
        analistaId: Number(data.analista), 
        marcaTq: data.marca_tanque,
        loteId: Number(data.lote),
        alcoolPercent: data.alcool ? Number(data.alcool) : null,
        densidade: data.densidade ? Number(data.densidade) : null,
        acidezTotalGL: data.acidez_total ? Number(data.acidez_total) : null,
        acidezVolatilGL: data.acidez_volatil
          ? Number(data.acidez_volatil)
          : null,
        so2TotalGL: data.so2_total ? Number(data.so2_total) : null,
        so2LivreGL: data.so2_livre ? Number(data.so2_livre) : null,
        turbidezNTU: data.turbidez ? Number(data.turbidez) : null,
        indiceCorIC: data.indice_cor ? Number(data.indice_cor) : null,
        ph: data.ph ? Number(data.ph) : null,
        acucarGL: data.acucar ? Number(data.acucar) : null,
      ...
      await api.post("/lab-wine", payload);
        ...
        >
          <View style={styles.formSection}>
            ...
            <Text style={styles.sectionSubtitle}>
              Preencha os dados da análise físico-química
            </Text>
          </View>
            ...
            <View style={styles.acucarRow}>
              ...
              <View style={styles.classificacaoContainer}>
                ...
                {classificacao ? (
                  <View
                    style={[
                      styles.classificacaoChip,
                      { backgroundColor: classificacao.color },
                    ]}
                  >
                  ...
                  <View style={styles.classificacaoPlaceholder}>
                    <Text style={styles.classificacaoPlaceholderText}>
                      Digite o valor
                    </Text>
                  </View>
                )}
