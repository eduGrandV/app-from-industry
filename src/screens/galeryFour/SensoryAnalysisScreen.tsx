...
import {
  SensoryAnalysisFormData,
  sensoryAnalysisSchema,
} from "../../types/galleryFour/sensoryAnalysisSchema";
...
export function SensoryAnalysisScreen() {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<SensoryAnalysisFormData>({
  ...
  const onSubmit = async (data: SensoryAnalysisFormData) => {
    ...
    try {
      ...
      await api.post("/sensory-analysis", payload);
    ...
    return (
      <View
        style={[
          ...
          hasError && styles.ratingContainerError,
        ]}
      >
        <Text style={styles.questionText}>
          O quanto você gostou ou desgostou de{" "}
          <Text style={styles.highlightText}>{question}</Text>?
        </Text>
        ...
        {hasError && (
          <Text style={styles.errorText}>Por favor, selecione uma nota.</Text>
        )}
      </View>
    );
  };
        ...
        >
          ...
          <View style={styles.header}>
            <View
              style={[
                ...
                { backgroundColor: isSuco ? "#FEF08A" : "#FBCFE8" },
              ]}
            >
            ...
            <View style={styles.rowMain}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.label}>
                  LOTE {isSuco ? "DO SUCO" : "DA GELEIA/DOCE"}
                </Text>
                <Controller
                  control={control}
                  name="lote"
              ...
              <View style={styles.infoCol}>
                {HEDONIC_SCALE.slice(0, 5).map((item) => (
                  <Text key={item.value} style={styles.infoText}>
                    <Text style={styles.infoNumber}>{item.value}</Text> -{" "}
                    {item.label}
                  </Text>
                ))}
              </View>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.sectionBody}>
              ...
              <RatingScale
                name="avaliacao_global"
                question={`sua AVALIAÇÃO GLOBAL`}
              />
            </View>
          </View>
