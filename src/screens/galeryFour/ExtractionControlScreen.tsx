...
import {
  extractionControlSchema,
  ExtractionControlFormData,
} from "../../types/galleryFour/extractionControlSchema";
  ...
  <View style={styles.innerCard}>
    ...
    <View style={[styles.row, { marginTop: 10 }]}>
      <InputGroup
        control={control}
        label="Hora Inicial"
        name={`${prefix}.hora_ini`}
        placeholder="00:00"
      />
      <InputGroup
        control={control}
        label="Hora Final"
        name={`${prefix}.hora_fim`}
        placeholder="00:00"
      />
    </View>
  </View>
...
export function ExtractionControlScreen() {
  ...
  const { control, handleSubmit, setValue, reset, watch } =
    useForm<ExtractionControlFormData>({
  ...
  const totalVolume = tamboresWatch.reduce(
    (acc, curr) => acc + (Number(curr.volume_L) || 0),
    0,
  );
  ...
  const onSubmit = async (data: ExtractionControlFormData) => {
    ...
    try {
      ...
      await api.post("/extraction", payload);
        ...
        >
            ...
            <View style={styles.sectionBody}>
                  ...
                  {showDatePicker && (
                    <DateTimePicker
                      ...
                      onChange={(e, selectedDate?: Date) => {
                        ...
                        if (selectedDate)
                          setValue("data_recepcao", selectedDate);
                      }}
                    />
                  )}
                </View>
              ...
              <View style={[styles.row, { marginTop: 12 }]}>
                <InputGroup
                  ...
                  name="hora_ini_extracao"
                  placeholder="00:00"
                />
                <InputGroup
                  control={control}
                  label="Fim Extração"
                  name="hora_fim_extracao"
                  placeholder="00:00"
                />
              </View>
              <View style={[styles.row, { marginTop: 12 }]}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.sectionBody}>
              <TempStepCard
                control={control}
                title="Tratamento Enzimático"
                prefix="temp_enzimatico"
              />
              <TempStepCard
                control={control}
                title="Pasteurização"
                prefix="temp_pasteurizacao"
              />
              <TempStepCard
                control={control}
                title="Envase"
          ...
          <View style={styles.section}>
            ...
            <View style={styles.sectionBody}>
              <AnaliseCard
                control={control}
                title="Análise da Uva"
                prefix="analise_uva"
              />
              <AnaliseCard
                control={control}
                title="Análise do Suco"
                prefix="analise_suco"
              />
            </View>
          </View>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.sectionBody}>
                ...
                <View key={field.id} style={styles.dynamicRow}>
                  <View style={styles.row}>
                    <InputGroup
                      control={control}
                      label={`Tambor Nº`}
                      name={`tambores.${index}.numero`}
                    />
                    <InputGroup
                      control={control}
                      label="Volume (L)"
                      name={`tambores.${index}.volume_L`}
                      keyboard="numeric"
                    />

                    {tamborFields.length > 1 && (
                      <TouchableOpacity
                        onPress={() => removeTambor(index)}
              ...
              <TouchableOpacity
                ...
                onPress={() => addTambor({ numero: "", volume_L: 0 })}
              >
                <MaterialIcons
          ...
          <View style={styles.card}>
            <InputGroup
              ...
              label="Observações"
              name="observacao"
              placeholder="Ocorrências no processo..."
            />
