...
export function PackagingMonitoringScreen() {
  ...
  const { control, handleSubmit } = useForm<PackagingMonitoringFormData>({
  ...
  const onSubmit = (data: PackagingMonitoringFormData) => {
    console.log("Monitoramento de Envase Salvo:", data);
    Alert.alert("Sucesso", "Monitoramento de equipamentos de envase registado!");
    navigation.goBack();
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {/* 1. DADOS GERAIS (Agora mais enxuto) */}
        <View style={styles.section}>
          ...
          <View style={styles.sectionBody}>
            <View style={styles.row}>
              ...
              <InputGroup label="Lote N°" name="lote" placeholder="Ex: L-001" />
            </View>
        ...
        {/* 2. REGISTROS (EQUIPAMENTOS + JORNADA NO FINAL) */}
        <View style={styles.section}>
            ...
            {registrosFields.map((field, index) => (
              <View key={field.id} style={styles.dynamicCard}>
                ...
                <View style={[styles.equipBlock, { borderTopWidth: 0, paddingTop: 0, marginTop: 0 }]}>
                  ...
                  <View style={styles.row}>
                    <InputGroup label="Horário da Leitura" name={`registros.${index}.horario`} placeholder="00:00" />
                    <InputGroup label="Observações" name={`registros.${index}.observacao`} flex={2} />
                  </View>
                </View>
                ...
                <View style={styles.equipBlock}>
                  ...
                  <View style={styles.row}>
                    <InputGroup label="Pressão (Kgf/cm²)" name={`registros.${index}.past_pressao`} keyboard="numeric" />
                    <InputGroup label="T. Água (°C)" name={`registros.${index}.past_temp_agua`} keyboard="numeric" />
                    <InputGroup label="T. Suco Saída (°C)" name={`registros.${index}.past_temp_suco`} keyboard="numeric" />
                  </View>
                </View>
                ...
                <View style={styles.equipBlock}>
                  ...
                  <View style={styles.row}>
                    <InputGroup label="Lavadora: T.(°C)" name={`registros.${index}.lav_temp`} keyboard="numeric" />
                    <InputGroup label="Envase: Bomba(Hz)" name={`registros.${index}.envase_bomba_hz`} keyboard="numeric" />
                  </View>
                  <View style={[styles.row, { marginTop: 10 }]}>
                    <InputGroup label="Envase: T. Atual(°C)" name={`registros.${index}.envase_temp_atual`} keyboard="numeric" />
                    <InputGroup label="Envase: T. Garrafa(°C)" name={`registros.${index}.envase_temp_garrafa`} keyboard="numeric" />
                  </View>
                </View>
                ...
                <View style={styles.equipBlock}>
                  ...
                  <View style={styles.row}>
                    <InputGroup label="Vazão (unid./h)" name={`registros.${index}.tamp_vazao`} keyboard="numeric" />
                    <InputGroup label="Perda Tampas" name={`registros.${index}.tamp_perda`} keyboard="numeric" />
                  </View>
                </View>

                {/* --- CONTROLE DE JORNADA E PRODUTO (AGORA NO FINAL DO CARTÃO) --- */}
                <View style={[styles.equipBlock, { backgroundColor: '#F0FDFA', marginHorizontal: -16, paddingHorizontal: 16, paddingBottom: 16, borderBottomLeftRadius: 12, borderBottomRightRadius: 12 }]}>
                  ...
                  <View style={styles.row}>
                    <InputGroup label="Modelo de Garrafa" name={`registros.${index}.modelo_garrafa`} placeholder="Ex: Vidro 1L" />
                  </View>
                  <View style={[styles.row, { marginTop: 10 }]}>
                    <InputGroup label="Início Processo" name={`registros.${index}.jornada_inicio`} placeholder="00:00" />
                    <InputGroup label="Fim Processo" name={`registros.${index}.jornada_fim`} placeholder="00:00" />
                  </View>
                  <View style={[styles.row, { marginTop: 10 }]}>
                    <InputGroup label="Almoço (Início)" name={`registros.${index}.jornada_almoco_ini`} placeholder="00:00" />
                    <InputGroup label="Almoço (Fim)" name={`registros.${index}.jornada_almoco_fim`} placeholder="00:00" />
                  </View>
                </View>
        ...
        <View style={styles.card}>
          ...
          <InputGroup label="Assinatura do Operador" name="assinatura_operador" placeholder="Nome / Visto do responsável" />
        </View>
